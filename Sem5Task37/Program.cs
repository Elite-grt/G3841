...
int[] ConvertArr(int[] arr)  // Принимаем  массив
{
    int[] resArr = new int[(arr.Length/2)+1]; // Создаем новый массив. Размер массива arr.Length/2)+1 тк идем до середины массива
    for (int i = 0; i < resArr.Length; i++) // Проходим по массиву resArr.Length
    {
        resArr[i]=arr[i]*arr[arr.Length-1-i]; // Обращаемся к элементу массиву, который равен arr[i]*arr[arr.Length-1-i]
    }
    return resArr;
}
