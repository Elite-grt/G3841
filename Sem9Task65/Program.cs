...
string RecLineMN(int m, int n)
{
    if (m >= n)
    {
        return n.ToString(); // преобразуем в строку
    }
    else
    {
...
//     if (10 >= 10)
//     {
//         return 10.ToString(); // преобразуем в строку
//     }
//     else
//     {
//         return m + " " + RecLineMN(m + 1, n);
...
// <тип данных> имя_перем = (условия)?<значение1>:<значение2>;
string res = numM<numN? RecLineMN(numM,numN):RecLineMN(numN,numM); // защита от дурака
PrintResult(res);
